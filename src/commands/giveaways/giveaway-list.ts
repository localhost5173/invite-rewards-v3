...
export default async function (interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const showActiveTranslation =
      languageData.giveaways.translations.showActive;
    const noActiveGiveawaysFoundTranslation =
      languageData.giveaways.translations.noActiveGiveawaysFound;
    const noEndedGiveawaysFoundTranslation =
      languageData.giveaways.translations.noEndedGiveawaysFound;

    const filterGiveaways = (activeOnly: boolean) =>
      guildGiveaways.filter((giveaway) =>
        activeOnly ? !giveaway.ended : giveaway.ended
      );
    ...
    const createGiveawayEmbed = async (index: number) => {
      ...
      return await Embeds.createEmbed(
        guildId,
        "giveaways.giveawayPage",
        replacements
      );
    };
    ...
    const updateEmbedMessage = async (
      ...
      const buttons = new ActionRowBuilder<ButtonBuilder>().addComponents(
        ...
        new ButtonBuilder()
          ...
          .setStyle(ButtonStyle.Primary)
          .setDisabled(
            giveaways.length === 0 || currentPage === giveaways.length - 1
          ),
        new ButtonBuilder()
        ...
        new ButtonBuilder()
          ...
          .setLabel(
            showActiveOnly ? showEndedTranslation : showActiveTranslation
          )
