...
export default async function () {
    try {
        ...
        for (const [_, guild] of guilds) {
            ...
            const smartLeaderboards = await db.leaderboards.getSmartLeaderboards(fullGuild.id);
                ...
                for (const smartLeaderboard of batch) {
                    ...
                    try {
                        ...
                        if (channel) {
                            ...
                            if (message) {
                                cs.dev(`Cached message ${messageId} in channel ${channelId} for guild ${guild.id}`);
                            }
                        }
                    }
                    catch (error) {
                        cs.error(`Error caching message ${messageId} in channel ${channelId} for guild ${guild.id}: ${error}`);
                        try {
                            await db.leaderboards.deleteSmart(fullGuild.id, channelId, messageId);
                            cs.dev(`Deleted smart leaderboard for message ${messageId} in channel ${channelId} for guild ${guild.id}`);
                        }
                        catch {
                            cs.error(`Error deleting smart leaderboard for message ${messageId} in channel ${channelId} for guild ${guild.id}`);
                        }
                    }
                }
