...
export default async function (guildMember) {
    try {
        ...
        const joinedUser = await db.invites.joinedUsers.getJoinedUser(guildId, guildMember.id);
...
async function sendFarewellMessage(guildMember, inviter, vanity) {
    const guildId = guildMember.guild.id;
    try {
        ...
        const { farewellChannelId, farewellMessage: serverFarewellMessage, farewellVanityMessage, farewellEmbed: serverFarewellEmbed, } = server;
        ...
        if (farewellChannelId) {
            const farewellChannel = guildMember.guild.channels.cache.get(farewellChannelId);
            if (farewellChannel && farewellChannel.isTextBased()) {
                ...
                if (farewellEmbed) {
                    const finalFarewellEmbed = replaceEmbedPlaceholders(farewellEmbed, guildMember, inviter, inviterInvites);
                    await sendServerFarewellEmbed(farewellChannel, finalFarewellEmbed);
                }
                else if (farewellMessage) {
                    const finalFarewellMessage = replacePlaceholders(farewellMessage, guildMember, inviter, inviterInvites);
                    await sendServerFarewellMessage(farewellChannel, finalFarewellMessage);
                }
            }
            else {
                cs.error(`Farewell channel not found or not text-based for guild: ${guildId}`);
            }
        }
        // Send farewell message to the user's DM if it exists
        if (dmFarewellEmbed) {
            const finalDmFarewellEmbed = replaceEmbedPlaceholders(dmFarewellEmbed, guildMember, inviter, inviterInvites);
            await sendDmFarewellEmbed(guildMember, finalDmFarewellEmbed);
        }
        else if (dmFarewellMessage) {
            const finalDmFarewellMessage = replacePlaceholders(dmFarewellMessage, guildMember, inviter, inviterInvites);
 ...
 */
async function sendDmFarewellMessage(guildMember, dmFarewellMessage) {
    try {
 ...
 */
async function sendDmFarewellEmbed(guildMember, dmFarewellEmbed) {
    try {
 ...
 */
async function sendServerFarewellMessage(farewellChannel, serverFarewellMessage) {
    try {
 ...
 */
async function sendServerFarewellEmbed(farewellChannel, serverFarewellEmbed) {
    try {
