import { ActionRowBuilder, GuildMemberRoleManager, InteractionCollector, TextInputBuilder, TextInputStyle, EmbedBuilder, ButtonBuilder, ButtonStyle, } from "discord.js";
...
export default async function (interaction) {
    ...
    if (!interaction.guild)
        return;
    try {
        const verificationObject = await db.verification.getVerification(interaction.guild.id);
        const isFakeUser = await db.invites.joinedUsers.isFakeUser(interaction.guild.id, interaction.user.id);
        if (isFakeUser) {
            await interaction.reply({
                embeds: [
                    await Embeds.createEmbed(interaction.guild.id, "verification.event.all.fakeUser"),
                ],
                ephemeral: true,
            });
            return;
        ...
        if (!verificationObject || !verificationObject.enabled) {
            await interaction.reply({
                embeds: [
                    await Embeds.createEmbed(interaction.guild.id, "verification.event.all.verificationDisabled"),
                ],
                ephemeral: true,
            });
            return;
        ...
        const { passed, embed } = await checkRolePermissions(interaction, verificationObject.roleId, member);
        if (!passed) {
            await interaction.reply({
                embeds: [embed],
                ephemeral: true,
        ...
        switch (verificationObject.type) {
            case "simple":
                await handleSimpleVerification(interaction, verificationObject.roleId, member);
                break;
            case "question":
                await handleQuestionVerification(interaction, verificationObject, member);
                break;
            case "pin":
            ...
            default:
                await interaction.reply({
                    embeds: [
                        await Embeds.createEmbed(interaction.guild.id, "verification.event.all.unknownType"),
                    ],
                    ephemeral: true,
                });
        }
        ...
        interaction.followUp({
            embeds: [
                await Embeds.createEmbed(interaction.guild.id, "verification.event.all.error"),
            ],
            ephemeral: true,
        });
    }
...
// Simple verification
async function handleSimpleVerification(interaction, roleId, member) {
    // Check if the bot has the necessary permissions to add the role
    const { passed, role, embed } = await checkRolePermissions(interaction, roleId, member);
    ...
    if (member.roles instanceof GuildMemberRoleManager) {
        ...
        const inviterId = await db.invites.joinedUsers.getInviterOfUser(guildId, userId);
...
async function handleQuestionVerification(interaction, verificationObject, member) {
    ...
    if (!roleId || !question || !correctAnswer) {
        await interaction.reply({
            embeds: [
                await Embeds.createEmbed(guildId, "verification.event.question.invalidConfig"),
            ],
            ephemeral: true,
        });
        return;
    ...
    // Check if the bot has the necessary permissions to add the role
    const { passed, embed } = await checkRolePermissions(interaction, roleId, member);
    if (!passed) {
        await interaction.reply({
            embeds: [embed],
            ephemeral: true,
    ...
    const filter = (i) => i.customId === "verification-modal" && i.user.id === interaction.user.id;
    ...
    collector.on("collect", async (modalInteraction) => {
        const userAnswer = modalInteraction.fields.getTextInputValue("verification-question");
        ...
        if (userAnswer.toLowerCase() === correctAnswer.toLowerCase()) {
            ...
            await modalInteraction.reply({
                embeds: [
                    await Embeds.createEmbed(guildId, "verification.event.question.incorrectAnswer"),
                ],
                ephemeral: true,
            });
        }
...
async function handlePinVerification(interaction, verificationObject, member) {
    const guildId = interaction.guild.id;
    const userId = member.user.id;
    // Check if the bot has the necessary permissions to add the role
    const { passed, embed: errorEmbed } = await checkRolePermissions(interaction, verificationObject.roleId, member);
    if (!passed) {
        await interaction.reply({
            embeds: [errorEmbed],
            ephemeral: true,
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "PIN", value: `\`${pin}\``, inline: true }, { name: "Input", value: "` `", inline: true })
        .setColor(0x00ff00);
    const rows = Array.from({ length: 10 }, (_, i) => new ButtonBuilder()
        .setCustomId(`digit_${i}`)
        .setLabel(`${i}`)
        .setStyle(ButtonStyle.Secondary));
    const actionRows = [
        new ActionRowBuilder().addComponents(rows.slice(0, 5)),
        new ActionRowBuilder().addComponents(rows.slice(5)),
    ];
    await interaction.reply({
        embeds: [embed],
    ...
    collector?.on("collect", async (i) => {
        if (!i.isButton())
            return;
        if (i.user !== interaction.user || !i.customId.includes("digit_"))
            return;
        ...
        if (userInput.length >= 4) {
            ...
            if (userInput === `${pin}`) {
                ...
                await member.roles.add(verificationObject.roleId);
            }
            else {
                await i.update({
                    components: [],
...
async function questionVerificationModal(question, guildId) {
