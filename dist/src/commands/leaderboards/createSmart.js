import { ChannelType, PartialGroupDMChannel, PermissionFlagsBits, } from "discord.js";
import { cs } from "../../utils/console/customConsole.js";
import { Helpers } from "../../utils/helpers/helpers.js";
import { db } from "../../utils/db/db.js";
import { Embeds } from "../../utils/embeds/embeds.js";
import { Leaderboards } from "../../utils/leaderboards/Leaderboards.js";
export default async function (interaction, leaderboardType) {
    try {
        if (!interaction.memberPermissions ||
            !interaction.memberPermissions.has(PermissionFlagsBits.ManageGuild)) {
            await interaction.reply({
                embeds: [
                    await Embeds.createEmbed(interaction.guildId, "leaderboards.smart.noPermissions"),
                ],
                ephemeral: true,
            });
            return;
        ...
        if (smartLeaderboards.length >= 4) {
            await interaction.followUp({
                embeds: [
                    await Embeds.createEmbed(guildId, "leaderboards.smart.maxLeaderboards"),
                ],
                ephemeral: true,
            });
            return;
        ...
        // Get the leaderboard
        const leaderboard = await db.leaderboards.getLeaderboard(guildId, leaderboardType);
        ...
        const embed = await Embeds.createEmbed(guildId, `leaderboards.smart.${leaderboardType}`, {
            leaderboard: leaderboardMessage,
        });
        const message = await channel.send({ embeds: [embed] });
        await db.leaderboards.saveSmart(guildId, channelId, message.id, leaderboardType);
        await interaction.followUp({
