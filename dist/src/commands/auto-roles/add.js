...
export default async function (interaction) {
    try {
        ...
        if (autoroles.includes(role.id)) {
            return interaction.followUp({
                embeds: [
                    await Embeds.createEmbed(interaction.guildId, "autoRoles.add.alreadyAutoRoleWarning", { role: `<@&${role.id}>` }),
                ],
                ephemeral: true,
            });
        }
        ...
        if (autoroles.length >= 3) {
            return interaction.followUp({
                embeds: [
                    await Embeds.createEmbed(interaction.guildId, "autoRoles.add.maxAutoRolesError"),
                ],
                ephemeral: true,
            });
        }
        ...
        return await interaction.followUp({
            embeds: [
                await Embeds.createEmbed(interaction.guildId, "autoRoles.add.success", {
                    role: `<@&${role.id}>`,
                }),
            ],
        });
