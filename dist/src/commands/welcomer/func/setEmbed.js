...
export default async function (interaction, type, location) {
    ...
    try {
        ...
        const jsonField = await Embeds.createTextField(guildId, "modals.welcomer.setEmbed.jsonField");
        jsonField.setCustomId("setEmbedJsonField");
        jsonField.setRequired(true);
        jsonField.setStyle(TextInputStyle.Paragraph);
        modal.addComponents(new ActionRowBuilder().addComponents(jsonField));
        ...
        try {
            ...
            const errorEmbed = await Embeds.createEmbed(guildId, "general.invalidJson");
        ...
        const totalLength = (embedData.title?.length || 0) +
            ...
            (embedData.fields?.reduce((acc, field) => acc + (field.name?.length || 0) + (field.value?.length || 0), 0) || 0);
        if (embedData.title?.length > 256 ||
            ...
            totalLength > 6000) {
            const errorEmbed = await Embeds.createEmbed(guildId, "general.embedTooLong");
            return modalInteraction.followUp({
                embeds: [errorEmbed],
                ephemeral: true,
            });
