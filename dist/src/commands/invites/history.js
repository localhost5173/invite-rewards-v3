...
export async function run({ interaction }) {
    ...
    try {
        ...
        if (!targetUser) {
            const noUserEmbed = await Embeds.createEmbed(guildId, "general.noUserProvided");
        ...
        if (history.length === 0) {
            const noHistoryEmbed = await Embeds.createEmbed(guildId, "history.noHistory", {
                user: `<@${targetUser.id}>`,
            });
        ...
        const historyEmbed = await Embeds.createEmbed(guildId, "history.success", {
            user: `<@${targetUser.id}>`,
        });
