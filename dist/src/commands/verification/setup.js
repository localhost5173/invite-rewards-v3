import { ActionRowBuilder, ChannelType, ModalBuilder, TextInputBuilder, TextInputStyle, EmbedBuilder, TextChannel, ButtonBuilder, ButtonStyle, } from "discord.js";
...
export default async function (interaction, type) {
    ...
    try {
        ...
        const buttonTitleField = new TextInputBuilder()
            ...
            .setRequired(false);
        modal.addComponents(new ActionRowBuilder().addComponents(titleField), new ActionRowBuilder().addComponents(descriptionField), new ActionRowBuilder().addComponents(thumbnailField), new ActionRowBuilder().addComponents(colorField), new ActionRowBuilder().addComponents(buttonTitleField));
        await interaction.showModal(modal);
        const modalInteraction = await interaction.awaitModalSubmit({
            time: 60000,
            filter: (i) => i.customId === "verification-setup-simple",
        });
        const title = modalInteraction.fields.getTextInputValue("verification-setup-simple-title");
        const description = modalInteraction.fields.getTextInputValue("verification-setup-simple-description");
        const thumbnail = modalInteraction.fields.getTextInputValue("verification-setup-simple-thumbnail");
        let color = modalInteraction.fields.getTextInputValue("verification-setup-simple-color");
        const buttonTitle = modalInteraction.fields.getTextInputValue("verification-setup-simple-button-title");
        ...
        const colorResolvable = colorParsed && colorParsed.model === "rgb"
            ? colorString.to.hex(colorParsed.value) // Assert to ColorResolvable type
            : "#FFFFFF"; // Default color if invalid, also asserted
        // Create the verification embed
        const embed = new EmbedBuilder();
        embed.setTitle(title ? title : "Verification");
        embed.setDescription(description ? description : "Please verify that you are a human.");
        ...
        switch (type) {
            ...
            case "question": {
                ...
                if (!question || !answer) {
                    return interaction.reply({
                        embeds: [
                            await Embeds.createEmbed(guildId, "verification.setup.noQuestionAnswer"),
                        ],
                        ephemeral: true,
                    });
                }
                await db.verification.enableQuestion(guildId, role.id, question, answer);
                break;
            }
            case "pin":
        ...
        cs.error("Error occurred while setting up the verification system: " + error);
