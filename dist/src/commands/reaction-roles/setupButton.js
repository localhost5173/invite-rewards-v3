...
export default async function (interaction) {
    ...
    if (!roles.length) {
        await interaction.followUp({
            embeds: [
                await Embeds.createEmbed(interaction.guildId, "reactionRoles.noRolesProvided"),
            ],
            ephemeral: true,
        });
        return;
    ...
    for (const role of roles) {
        ...
        if (role.managed) {
            return interaction.followUp({
                embeds: [
                    await Embeds.createEmbed(interaction.guildId, "roles.managedRoleAssignError", { role: `<@&${role.id}>` }),
                ],
                ephemeral: true,
            });
        }
        ...
        if (!interaction.guild.members.me ||
            ...
            return interaction.followUp({
                embeds: [
                    await Embeds.createEmbed(interaction.guildId, "roles.hierarchyRoleAssignError", { role: `<@&${role.id}>` }),
                ],
                ephemeral: true,
            });
        }
        if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageRoles)) {
            return interaction.followUp({
                embeds: [
                    await Embeds.createEmbed(interaction.guildId, "roles.noManageRolesPermissionError"),
                ],
                ephemeral: true,
            });
        }
    ...
    roles.forEach((role, index) => {
        const roleText = interaction.options.getString(`role-${index + 1}-text`) || role.name;
        const roleStyle = interaction.options.getString(`role-${index + 1}-style`) || "PRIMARY";
        let buttonStyle;
