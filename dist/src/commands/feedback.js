import { TextInputStyle, ActionRowBuilder, } from "discord.js";
...
export async function run({ interaction }) {
    ...
    try {
        ...
        const actionRow1 = new ActionRowBuilder().addComponents(feedbackType);
        const actionRow2 = new ActionRowBuilder().addComponents(feedbackInput);
        const actionRow3 = new ActionRowBuilder().addComponents(satisfactionInput);
        // Add the ActionRows to the modal
        modal.addComponents(actionRow1, actionRow2, actionRow3);
        // Show the modal
        await interaction.showModal(modal);
        // Handle modal submission
        const filter = (i) => i.customId === "feedback" && i.user.id === interaction.user.id;
        // Collector to get the feedback from the modal
        interaction
            .awaitModalSubmit({ filter, time: 300000 }) // Collect feedback within 5 minutes
            .then(async (modalInteraction) => {
            const feedbackTypeValue = modalInteraction.fields.getTextInputValue("feedbackType");
            const feedback = modalInteraction.fields.getTextInputValue("feedbackInput");
            const satisfactionValue = modalInteraction.fields.getTextInputValue("satisfaction") ||
                "No rating given";
            ...
            const webhookUrl = "https://discord.com/api/webhooks/1292878246878249044/ufHhU2CMlyaWvgxZjHpkMiGu9T1NKXyVfX-f-55I4IrJrEi_hOYccne1fM2UzrDiAhbX";
