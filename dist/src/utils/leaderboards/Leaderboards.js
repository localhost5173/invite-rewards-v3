...
export class Leaderboards {
    static async updateLeaderboards(guildId, userId) {
        try {
            const newAllTimeInviteCount = await db.invites.userInvites.getRealAndBonusInvites(guildId, userId, "alltime");
            const newDailyInviteCount = await db.invites.userInvites.getRealAndBonusInvites(guildId, userId, "daily");
            const newWeeklyInviteCount = await db.invites.userInvites.getRealAndBonusInvites(guildId, userId, "weekly");
            const newMonthlyInviteCount = await db.invites.userInvites.getRealAndBonusInvites(guildId, userId, "monthly");
            // Track if any leaderboard type has changed
            let top10Changed = false;
            // Update each leaderboard type
            top10Changed =
                (await updateLeaderboard(guildId, userId, newDailyInviteCount, "daily")) || top10Changed;
            top10Changed =
                (await updateLeaderboard(guildId, userId, newWeeklyInviteCount, "weekly")) || top10Changed;
            top10Changed =
                (await updateLeaderboard(guildId, userId, newMonthlyInviteCount, "monthly")) || top10Changed;
            top10Changed =
                (await updateLeaderboard(guildId, userId, newAllTimeInviteCount, "alltime")) || top10Changed;
    ...
    static async formatLeaderboardMessage(guildId, entries) {
        let message = "";
...
async function updateLeaderboard(guildId, userId, inviteCount, leaderboardType) {
    // Retrieve current top 25 entries
    const topEntries = await db.leaderboards.getTopEntries(guildId, leaderboardType);
    ...
    const qualifiesForTop25 = topEntries.length < 25 ||
        inviteCount > topEntries[topEntries.length - 1].inviteCount;
    if (qualifiesForTop25) {
        // Upsert user's invite count in the top 25
        await db.leaderboards.updateEntry(guildId, userId, leaderboardType, inviteCount);
    ...
    const newTopEntries = await db.leaderboards.getTopEntries(guildId, leaderboardType);
    const top10Changed = hasTop10Changed(topEntries, newTopEntries);
    return top10Changed;
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function hasTop10Changed(oldEntries, newEntries) {
    for (let i = 0; i < 10; i++) {
...
async function updateSmartLeaderboards(guildId) {
    ...
    for (const smartLeaderboard of smartLeaderboards) {
        const leaderboard = await db.leaderboards.getLeaderboard(guildId, smartLeaderboard.leaderboardType);
        ...
        const embed = await Embeds.createEmbed(guildId, `leaderboards.smart.${smartLeaderboard.leaderboardType}`, {
