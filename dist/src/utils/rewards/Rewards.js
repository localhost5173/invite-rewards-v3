...
export class Rewards {
    static async handleGiveRewards(guildId, userId) {
        try {
            const rewards = await db.rewards.getRewards(guildId);
            const inviteCount = await db.invites.userInvites.getRealAndBonusInvites(guildId, userId, "alltime");
            if (!rewards)
                return;
            for (const reward of rewards) {
                ...
                if (reward.claimedBy.includes(userId))
                    continue;
                if (inviteCount < reward.inviteThreshold)
                    continue;
                // Give the reward
...
async function giveRoleReward(reward, guildId, userId) {
    try {
...
async function giveMessageReward(reward, guildId, userId) {
    try {
        const guild = await client.guilds.fetch(guildId);
        const guildMember = await guild.members.fetch(userId);
...
async function giveMessageStoreReward(reward, guildId, userId) {
    try {
        const guild = await client.guilds.fetch(guildId);
        const guildMember = await guild.members.fetch(userId);
        // Check if the bot has permission to send DMs
        ...
        let message = await db.rewards.shiftFirstMessageFromStore(guildId, reward.rewardName);
        if (!message) {
...
async function unclaimRoleReward(reward, guildId, userId) {
    try {
        const guild = await client.guilds.fetch(guildId);
        const guildMember = await guild.members.fetch(userId);
        const bot = guild.members.me;
...
function replaceMessagePlaceholders(guild, inviteThreshold, message) {
    return message
