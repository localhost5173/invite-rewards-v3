...
class view {
    static async success(guildId, autoRoles, guildIcon) {
...
class add {
    static async alreadyAutoRoleWarning(guildId, roleId) {
        const language = await db.languages.getLanguage(guildId);
        const languageData = await import(`../../languages/${language}.json`);
        return new EmbedBuilder()
            .setColor(Embeds.color.warn)
            .setTitle(languageData.autoRoles.add.alreadyAutoRoleWarning.title)
            .setDescription(languageData.autoRoles.add.alreadyAutoRoleWarning.description.replace("{role}", `<@&${roleId}>`));
    }
    static async success(guildId, roleId) {
        const language = await db.languages.getLanguage(guildId);
        const languageData = await import(`../../languages/${language}.json`);
        return new EmbedBuilder()
            .setColor(Embeds.color.success)
            .setTitle(languageData.autoRoles.add.success.title)
            .setDescription(languageData.autoRoles.add.success.description.replace("{role}", `<@&${roleId}>`));
    }
}
class remove {
    static async notAutoRoleError(guildId, roleId) {
        const language = await db.languages.getLanguage(guildId);
        const languageData = await import(`../../languages/${language}.json`);
        return new EmbedBuilder()
            .setColor(Embeds.color.error)
            .setTitle(languageData.autoRoles.remove.notAutoRoleError.title)
            .setDescription(languageData.autoRoles.remove.notAutoRoleError.description.replace("{role}", `<@&${roleId}>`));
    }
    static async success(guildId, roleId) {
        const language = await db.languages.getLanguage(guildId);
        const languageData = await import(`../../languages/${language}.json`);
        return new EmbedBuilder()
            ...
            .setDescription(languageData.autoRoles.remove.success.description.replace("{role}", `<@&${roleId}>`));
    }
}
class assign {
    static async noManageRolesPermissionError(guild, dm = false) {
        const guildId = guild.id;
        const language = await db.languages.getLanguage(guildId);
        const languageData = await import(`../../languages/${language}.json`);
        return new EmbedBuilder()
            .setColor(Embeds.color.error)
            .setTitle(languageData.autoRoles.assign.noManageRolesPermissionError.title)
            .setDescription(dm
            ? languageData.autoRoles.assign.noManageRolesPermissionError.dmDescription.replace("{serverName}", guild.name)
    ...
    static async hierarchyRoleAssignError(guild, dm = false) {
        const guildId = guild.id;
        const language = await db.languages.getLanguage(guildId);
        const languageData = await import(`../../languages/${language}.json`);
        return new EmbedBuilder()
            ...
            .setDescription(dm
            ? languageData.autoRoles.assign.hierarchyRoleAssignError.dmDescription.replace("{serverName}", guild.name)
