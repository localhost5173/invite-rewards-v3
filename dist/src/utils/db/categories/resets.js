...
export class resets {
    ...
    static async setLastDailyReset() {
        await ResetsModel.findOneAndUpdate({ resetType: "daily" }, { lastReset: Date.now() }, { upsert: true });
    }
    static async setLastWeeklyReset() {
        await ResetsModel.findOneAndUpdate({ resetType: "weekly" }, { lastReset: Date.now() }, { upsert: true });
    }
    static async setLastMonthlyReset() {
        await ResetsModel.findOneAndUpdate({ resetType: "monthly" }, { lastReset: Date.now() }, { upsert: true });
    }
}
